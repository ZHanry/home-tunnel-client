# Client installation and operations
...
- `home-tunnel-agent` is built from the same restricted FRP 0.70.1 source and
  `agent/main.go` validation surface as the Windows release. It accepts
...
ARCH=arm64 ./packaging/build-release.sh

# macOS packages (cross-compile from any build host)
ARCH=amd64 ./packaging/macos/build-release.sh
...
On the target Linux machine, download the archive from GitHub Release `v5.0.0`.
The tarball is still named `5.0.0` because Stable promotes the RC assets
without rebuilding:

```sh
tar -xzf home-tunnel-linux-5.0.0-amd64.tar.gz
cd home-tunnel-linux-5.0.0-amd64
sudo ./install.sh
...
```sh
tar -xzf home-tunnel-macos-5.0.0-arm64.tar.gz
cd home-tunnel-macos-5.0.0-arm64
sudo ./install.sh
sudo home-tunnel-enroll
```
...
#   powershell ./packaging/windows/build-gui.ps1
```
